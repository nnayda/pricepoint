"""Raleigh police incident collector."""
    ...
    with engine.begin() as conn:
        ...
        conn.execute(
            text(
                ...
                f"{TABLE_NAME}_geom ON {TABLE_NAME} USING GIST (geometry);"
            )
        )
        ...
        if "reported_date" in gdf.columns:
            conn.execute(
                text(f"CREATE INDEX idx_{TABLE_NAME}_date ON {TABLE_NAME} (reported_date);")
            )

        if "crime_category" in gdf.columns:
            conn.execute(
                text(f"CREATE INDEX idx_{TABLE_NAME}_category ON {TABLE_NAME} (crime_category);")
            )
