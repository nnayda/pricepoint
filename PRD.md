...
- [ ] Add `valkey` service to `docker-compose.yml` (`valkey/valkey:8-alpine`, port 6379, profile `infra`, healthcheck)
- [ ] Add `valkeydata:` named volume
- [ ] Add `VALKEY_URL=redis://valkey:6379/0` to `.env.example` and `.env`
...
- [ ] Add `redis[hiredis]>=5.0,<6` to `pyproject.toml` dependencies
- [ ] Run `uv sync` to update lockfile
- [ ] Add `valkey_url: str | None = None` to `Settings` class in `src/pricepoint/config/settings.py`
- [ ] Add async `get_valkey()` dependency to `src/pricepoint/api/dependencies.py` (reads from `app.state.valkey_pool`, yields `None` when unavailable)
- [ ] Update `lifespan()` in `src/pricepoint/api/main.py` to init/close Valkey connection pool on `app.state`
...
- [ ] Create `src/pricepoint/api/schemas/geocode.py` — `GeocodeResult` (display_name, lat, lon, place_id, osm_type, osm_id, boundingbox) and `GeocodeResponse` (results, cached)
- [ ] Create `src/pricepoint/api/routes/geocode.py` — `GET /geocode` with params `q: str`, `limit: int = 5`
...
- [ ] Register `geocode.router` at `/api` prefix in `src/pricepoint/api/main.py`
...
- [ ] Create `tests/unit/test_api/test_geocode.py`
...
- [ ] Uses existing `client` fixture from `tests/conftest.py` + `app.dependency_overrides`
...
- [ ] `npm install tailwindcss @tailwindcss/vite` (NO postcss, NO autoprefixer, NO JS config file)
- [ ] Add `import tailwindcss from "@tailwindcss/vite"` and `tailwindcss()` to plugins in `frontend/vite.config.ts`
- [ ] Optionally add to `frontend/vitest.config.ts` plugins
...
- [ ] Download Plus Jakarta Sans woff2 files (400, 500, 600, 700) to `frontend/src/assets/fonts/`
...
- [ ] Add `import "./index.css"` to `frontend/src/main.tsx` (before App import)
- [ ] Update `<title>` to "PricePoint" in `frontend/index.html`
...
- [ ] Add `GeocodeResult` and `GeocodeResponse` interfaces to `frontend/src/types/index.ts`
- [ ] Create `frontend/src/types/view-transitions.d.ts` — type declaration for `document.startViewTransition`
- [ ] Create `frontend/src/utils/viewTransition.ts` — `startViewTransition()` wrapper with progressive enhancement fallback
...
- [ ] Create `frontend/src/services/geocode.ts` — Axios client calling `GET /api/geocode` with `q` and `limit` params (follows pattern of existing `frontend/src/services/api.ts`)
- [ ] Create `frontend/src/hooks/useDebounce.ts` — generic value debounce with `setTimeout`/`clearTimeout`
- [ ] Create `frontend/src/hooks/useGeocode.ts`:
...
- [ ] Create `frontend/src/components/SearchBar/SearchBar.tsx`
...
- [ ] Verify no remaining imports (`grep -r "DashboardPage" frontend/src/`)
- [ ] Keep: MapView, PropertyCard, ForecastPage, all existing tests
...
- [ ] Update `frontend/src/test/setup.ts` — add `import "vitest-axe/extend-expect"`
- [ ] Create `frontend/src/hooks/__tests__/useDebounce.test.ts` — timer behavior, reset on change, custom delay
- [ ] Create `frontend/src/hooks/__tests__/useGeocode.test.ts` — debounce integration, race conditions, error state, min 3 chars, loading state
- [ ] Create `frontend/src/services/__tests__/geocode.test.ts` — axios mock, param verification, default/custom limit
- [ ] Create `frontend/src/components/SearchBar/__tests__/SearchBar.test.tsx` — both variants, autocomplete flow, keyboard nav (ArrowUp/Down/Enter/Escape), onSelect callback, default navigation, loading/empty/error states, a11y roles (combobox/listbox/option), vitest-axe no violations
- [ ] Create `frontend/src/pages/__tests__/LandingPage.test.tsx` — heading, subtitle, hero SearchBar, axe check
- [ ] Create `frontend/src/components/NavBar/__tests__/NavBar.test.tsx` — home link href, compact SearchBar, nav landmark, axe check
- [ ] Create `frontend/src/pages/__tests__/ResultsPage.test.tsx` — URL param display, dynamic map render, missing params fallback, axe check (uses `MemoryRouter` with `initialEntries`)
...
- [ ] `cd frontend && npm run build` — no TypeScript or build errors
- [ ] `cd frontend && npm run lint && npm run format:check` — ESLint + Prettier pass
- [ ] `cd frontend && npm test` — all tests pass
- [ ] `make lint` — Ruff passes on backend
- [ ] `make test-unit` — backend unit tests pass
- [ ] `uv run mypy src/pricepoint/` — no type errors
- [ ] Manual: `/` shows centered "PricePoint" + hero search, no nav
- [ ] Manual: Type address → autocomplete via backend → select → `/results` with page cross-fade
- [ ] Manual: Results page has styled card + dynamic map + glassmorphism nav bar
- [ ] Manual: Nav scroll increases opacity/shadow
- [ ] Manual: `/forecast` still works; `/dashboard` redirects to `/`
- [ ] Manual: Mobile (375px width) — nav and search usable
- [ ] Manual: Network tab — no Google Fonts requests
