...
class TestPromoteModel:
    ...
    def test_promote_model_calls_transition(self, mock_client_cls: MagicMock) -> None:
        mock_client = MagicMock()
        mock_client_cls.return_value = mock_client

        promote_model(model_name="test-model", version=3, stage="Staging")
        mock_client.transition_model_version_stage.assert_called_once_with(
            name="test-model", version=3, stage="Staging"
        )

    @patch("mlflow.tracking.MlflowClient")
    def test_promote_model_default_stage(self, mock_client_cls: MagicMock) -> None:
        mock_client = MagicMock()
        mock_client_cls.return_value = mock_client
