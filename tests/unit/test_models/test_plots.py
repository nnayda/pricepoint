...
class TestPlotHelpers:
    ...
    def test_cv_fold_comparison(self, tmp_path: Path) -> None:
        ...
        path = _plot_cv_fold_comparison(cv_metrics=cv, output_dir=tmp_path, filename="cv.png")
        assert path is not None
        assert path.stat().st_size > 0

    def test_cv_fold_comparison_none(self, tmp_path: Path) -> None:
        path = _plot_cv_fold_comparison(cv_metrics=None, output_dir=tmp_path, filename="cv.png")
        assert path is None
    ...
    def test_learning_curves(self, eval_data, tmp_path: Path) -> None:
        ...
        path = _plot_learning_curves(
            model=model, x_test=x_test, y_true=y_true, output_dir=tmp_path, filename="lc.png"
        )
        assert path is not None
        assert path.stat().st_size > 0
